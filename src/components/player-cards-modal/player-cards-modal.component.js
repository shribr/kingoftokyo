...
export function build({ selector }) {
  ...
  root.innerHTML = `<div class="ppcm-frame" data-frame>
    <div class="ppcm-header" style="background:#f4f4f4;color:#000;border-bottom:1px solid#ccc;">
    ...
    <div class="ppcm-body" data-body style="background:#fff;color:#000;max-height:55vh;overflow:auto;padding:14px;"></div>
  </div>`;
...
export function update(root) {
  ...
  if (!player.cards.length) {
    body.innerHTML = `<div class="ppcm-empty" style="text-align:center;padding:30px 10px;color:#333;font-size:13px;">You currently have no power cards.<br/><span style="opacity:.7;">Buy cards from the shop to gain special abilities.</span></div>`;
  } else {
    body.innerHTML = `<div class="ppcm-cards-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;">${player.cards.map(c => renderOwnedCard(c)).join('')}</div>`;
  }
}
...
function renderOwnedCard(card) {
  ...
  return `<article class="ppcm-card-detail" data-card-id="${card.id}" data-rarity="${rarity}" ${isDarkEdition?'data-dark="1"':''} style="border:1px solid #ccc;border-radius:8px;background:#fafafa;color:#000;display:flex;flex-direction:column;gap:6px;padding:10px;box-shadow:0 2px 4px rgba(0,0,0,.07);">
