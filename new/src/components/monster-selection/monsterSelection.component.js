/** monsterSelection.component.js (renamed from setup.component.js) */
...
export function build({ selector, dispatch, getState }) {
  ...
  if (t.closest('[data-action="profiles"]')) { dispatch(uiMonsterProfilesOpen()); dispatch(uiMonsterSelectionClose()); inst.root.classList.add('hidden'); return; }
    ...
    if (t.closest('[data-player-count]')) { const li = t.closest('[data-player-count]'); const n = parseInt(li.getAttribute('data-player-count'), 10); if (!Number.isNaN(n)) inst._local.playerCount = n; while (inst._local.selected.size > n) { const last = Array.from(inst._local.selected).pop(); inst._local.selected.delete(last); } root.querySelector('[data-dropdown]')?.classList.remove('open'); render(inst); return; }
    const card = t.closest('[data-monster-card]');
    if (card) { const id = card.getAttribute('data-id'); const idx = inst._local.slots.indexOf(id); if (idx >= 0) { inst._local.slots[idx] = null; } else { const empty = inst._local.slots.findIndex(s => !s); if (empty >= 0) inst._local.slots[empty] = id; } syncSelectedFromSlots(inst); render(inst); return; }
    const slotEl = t.closest('[data-slot-index]');
    if (slotEl && slotEl.hasAttribute('data-has-monster')) { const si = parseInt(slotEl.getAttribute('data-slot-index'), 10); if (!Number.isNaN(si)) { inst._local.slots[si] = null; syncSelectedFromSlots(inst); render(inst); return; } }
  });

  root.addEventListener('dragstart', (e) => { const card = e.target.closest('[data-monster-card]'); const slot = e.target.closest('[data-slot-index]'); if (card) { const id = card.getAttribute('data-id'); if (!id) return; e.dataTransfer.setData('text/plain', id); e.dataTransfer.setData('application/x-monster-id', id); e.dataTransfer.effectAllowed = 'move'; } else if (slot && slot.hasAttribute('data-has-monster')) { const si = parseInt(slot.getAttribute('data-slot-index'), 10); const id = inst._local.slots[si]; if (!id) return; e.dataTransfer.setData('text/plain', id); e.dataTransfer.setData('application/x-monster-id', id); e.dataTransfer.setData('application/x-from-slot', String(si)); e.dataTransfer.effectAllowed = 'move'; } });
  ...
  root.addEventListener('drop', (e) => { const slot = e.target.closest('[data-slot-index]'); if (!slot) return; e.preventDefault(); slot.classList.remove('drag-over'); const id = e.dataTransfer.getData('application/x-monster-id') || e.dataTransfer.getData('text/plain'); if (!id) return; const si = parseInt(slot.getAttribute('data-slot-index'), 10); if (Number.isNaN(si)) return; const fromSlot = e.dataTransfer.getData('application/x-from-slot'); if (fromSlot) { const fIdx = parseInt(fromSlot, 10); if (!Number.isNaN(fIdx) && fIdx !== si) inst._local.slots[fIdx] = null; } const currentIdx = inst._local.slots.indexOf(id); if (currentIdx >= 0 && currentIdx !== si) inst._local.slots[currentIdx] = null; inst._local.slots[si] = id; syncSelectedFromSlots(inst); render(inst); });
...
export function update(ctx) {
  ...
  const open = st?.ui?.monsterSelection?.open; // updated slice key
  ...
  if (!inst._local._initialized) { inst._local.playerCount = 4; inst._local.slots = new Array(inst._local.playerCount).fill(null); inst._local.selected = new Set(); inst._local._initialized = true; inst._local.prevMonsterCount = 0; }
  render(inst, st);
}
...
function render(inst, fullState) {
  ...
  if (monsters.length && monsters.some(m => !m.image)) { if (!inst._local._warnedMissingImages) { console.warn('[monsterSelection.component] Some monsters missing image paths:', monsters.filter(m => !m.image).map(m => m.id)); inst._local._warnedMissingImages = true; } }
  if (!Array.isArray(inst._local.slots) || inst._local.slots.length !== inst._local.playerCount) { const old = inst._local.slots || []; const next = new Array(inst._local.playerCount).fill(null); for (let i = 0; i < next.length; i++) next[i] = old[i] || null; inst._local.slots = next; syncSelectedFromSlots(inst); }
  const grid = root.querySelector('[data-setup-grid]');
  if (grid) { grid.innerHTML = monsters.map(m => card(m, inst._local.selected.has(m.id))).join(''); grid.querySelectorAll('[data-monster-card]').forEach(el => { const id = el.getAttribute('data-id'); const selected = inst._local.selected.has(id); el.setAttribute('draggable', String(!selected)); if (selected) el.classList.add('is-unavailable'); else el.classList.remove('is-unavailable'); }); }
  const sidebar = root.querySelector('[data-sidebar]');
  if (sidebar) { const slotHTML = inst._local.slots.map((id, i) => { if (id) { const m = monsters.find(mm => mm.id === id); return miniCard(m, i, i === 0); } if (i === 0) return humanPlaceholderCard(i); return cpuCard(i); }).join(''); sidebar.innerHTML = `<div class="mini-list">${slotHTML}</div>`; inst._local.slots.forEach((id, i) => { if (!id) return; const m = monsters.find(mm => mm.id === id); const el = sidebar.querySelector(`[data-slot-index="${i}"]`); if (el && m && m.color) el.style.setProperty('--mini-bg', m.color + '22'); }); }
  ...
  if (startBtn) { startBtn.textContent = remaining > 0 ? `ASSIGN ${remaining} MORE MONSTER${remaining>1?'S':''}` : 'START GAME'; if (remaining > 0) startBtn.setAttribute('disabled', ''); else startBtn.removeAttribute('disabled'); }
}
...
function randomFill(inst) { const st = inst.getState?.(); const mons = selectMonsters(st); const pool = mons.map(m => m.id).filter(id => !inst._local.selected.has(id)); while (inst._local.slots.some(s => !s) && pool.length) { const i = Math.floor(Math.random() * pool.length); const id = pool.splice(i, 1)[0]; const empty = inst._local.slots.findIndex(s => !s); if (empty >= 0) inst._local.slots[empty] = id; } syncSelectedFromSlots(inst); }
