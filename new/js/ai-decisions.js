...
class AIDecisionEngine {
	...
	/* ---------------------------------------------------------
	 * State Extraction / Goal Handling
	 * --------------------------------------------------------- */
	_extractState(dice, rollsRemaining, player, gameState){
		...
		const numbers = ['one','two','three'].map(face => ({
			face,
			count: counts[face],
			faceValue: face === 'one' ? 1 : face === 'two' ? 2 : 3,
			pair: counts[face] === 2,
			formed: counts[face] >= 3
		}));
		...
		const effectiveRollsRemaining = rollsRemaining + (cardSummary.extraReroll>0 ? 1:0);
		let shopCards = [];
		try {
		...
		return {
			...
			cardSummary,
			player,
			rollsRemaining: effectiveRollsRemaining,
			origRolls: rollsRemaining,
			shopCards
		};
	}
	...
	_selectOrMaintainGoal(state, player){
		// Maintain existing if still viable
		if (player._aiTurnGoal){
			const face=player._aiTurnGoal.face;
			if (state.counts[face] > 0){
				if (face==='one') {
					const twoCount=state.counts.two; const threeCount=state.counts.three;
		...
		// Opportunistic early chase for 3s
	...
	_scoreDice(state, goal, player){
		...
		const tokyoSelf   = !!player.isInTokyo;
		const enemyTokyo  = state.tokyoOccupant && state.tokyoOccupant.id !== player.id ? state.tokyoOccupant : null;

		const arr=[];
		let energyUrgencyBonus=0;
		let portfolioDesiredFeature=null;

		// Portfolio synergy driver
		if (state.shopCards && state.shopCards.length) {
			try {
				if (typeof this.optimizePowerCardPortfolio === 'function') {
					...
					if (plan && plan.cards && plan.cards.length) {
						...
						const feats=this._extractCardFeatures(plan.cards[0]);
						portfolioDesiredFeature = featurePriority.find(f=> feats.has(f)) || null;
					}
				}
			} catch(e){}
			// Energy urgency analysis
			...
			if (impactful.length){
				...
				if (top.shortfall>0){
					energyUrgencyBonus = Math.max(4,(top.impactScore/10)) * (top.shortfall<=2 ? 1.4:1);
				}
			}
		}
		...
		state.dice.forEach((face,index)=>{
			...
			if (face==='one'||face==='two'||face==='three'){
				...
				if (count>=3){
					score = CFG.formedSetBase + count;
					locked=true;
				} else if (count===2){
					score = CFG.pairScoreBase + (face==='three'?18: face==='two'?10:5);
				} else {
					score = face==='three'? CFG.singleThree: face==='two'? CFG.singleTwo: CFG.singleOne;
					if (face==='one' && (counts.three>=2 || counts.two>=2)) score*=0.3;
				}
				...
				if (tokyoEmpty && !tokyoSelf && player.health >=6) baseAtk += 6 + (personality.aggression-3)*2;
				if (enemyTokyo){
					if (enemyTokyo.health<=5) baseAtk+=5;
					if (enemyTokyo.victoryPoints>=10) baseAtk+=5;
				}
				if (tokyoSelf && player.health <=5) baseAtk -= 6;
				score = baseAtk * aggressionAdj;
				if (cardSummary.attackBoost) score += 6*cardSummary.attackBoost;
				if (player.isInTokyo) score -=4;
				if (goal && counts[goal.face]===2) score*=0.75;
			} else if (face==='energy') {
				...
				if (portfolioDesiredFeature && ['extraDie','extraReroll'].includes(portfolioDesiredFeature)) score*=1.15;
	...
	_projectTwoRollEV(state, keepIndices){
		try {
			...
			if (AIDecisionProfiler.enabled){
				...
				if (s){
					if (s.avg<12) baseTrials=110;
					else if (s.avg<18) baseTrials=80;
					else if (s.avg>35) baseTrials=40;
					else if (s.avg>50) baseTrials=25;
				}
			}
			...
			return {
				tripleChance: +(aggregate.tripleGain/TRIALS).toFixed(3),
				avgAttack: +(aggregate.attack/TRIALS).toFixed(2),
				avgEnergy: +(aggregate.energy/TRIALS).toFixed(2),
				avgHeal:   +(aggregate.heal/TRIALS).toFixed(2)
			};
		} catch(e){
			return null;
		}
	...
	_computeSetEV(state, goal){
		...
		const committed=state.numbers.filter(n=>n.formed).reduce((a,n)=>a+n.count,0);
		const free=Math.max(0, effectiveDiceCount - committed);
			...
			if (n.count===2){
				...
				if (free>=2 || (free>=1 && (state.cardSummary.extraReroll>0 || extraDie>0))){
					const expectedMatches = Math.min(2,(free * R)/6);
						const ev5 = expectedMatches>1? (expectedMatches-1)*0.75 : 0;
						if (ev5>0) items.push({ face:n.face, type:'fiveKindChase', ev:ev5 });
				}
			} else if (n.count===4){
				if (free>0){
					const p5 = 1 - Math.pow(5/6, free * R * 0.65);
		...
		if (R>0 && free>0){
			...
			const attackEV=expPerFace * 0.9 * threatFactor;
			...
			if (additive>0){
				items.push({ face:'*', type:'additiveFuture', ev:additive });
				total+=additive;
			}
		}
		return { items, total };
	 ...
	 * Branch Simulation
	 * --------------------------------------------------------- */
	_simulateBranches(state){
		try {
			const counts=state.counts;
			const groups=['one','two','three'].map(f=>({face:f,count:counts[f]})).filter(g=>g.count>0).sort((a,b)=> b.count - a.count || (b.face==='three'?1:-1));
			const specials=state.dice.map((f,i)=>({f,i})).filter(d=> !['one','two','three'].includes(d.f));
			const branches=[];
			const primary=groups[0]?.face;
			const baseKeep=[];
			state.dice.forEach((f,i)=>{ if (groups.some(g=>g.face===f && g.count>=3)) baseKeep.push(i); });
			if (primary){ state.dice.forEach((f,i)=>{ if (f===primary && !baseKeep.includes(i)) baseKeep.push(i); }); }
			branches.push({ tag:'baseline', kept:baseKeep.slice() });
			if (specials.length){
				const sortedSpec=specials.slice().sort((a,b)=>{ const pr={ heart:1, energy:2, attack:3 }; return (pr[a.f]||5)-(pr[b.f]||5); });
				const alt=baseKeep.filter(i=> !sortedSpec.some(s=>s.i===i));
				branches.push({ tag:'free_one_special', kept:alt });
			}
			if (groups.length>1 && groups[1].count===2){
				const secondary=groups[1].face;
				const secondKeep=[];
				state.dice.forEach((f,i)=>{ if (f===secondary) secondKeep.push(i); });
				groups.forEach(g=>{ if (g.count>=3){ state.dice.forEach((f,i)=>{ if (f===g.face && !secondKeep.includes(i)) secondKeep.push(i); }); } });
				branches.push({ tag:'secondary_pair_focus', kept:secondKeep });
			}
			const numbersOnly=[];
			state.dice.forEach((f,i)=>{ if(['one','two','three'].includes(f)) numbersOnly.push(i); else if (f==='heart' && state.player.health<=CFG.healingCriticalHP) numbersOnly.push(i); });
			branches.push({ tag:'numbers_only', kept:numbersOnly });

			const evaluated = branches.map(br=>{
				const keptFaces=br.kept.map(i=>state.dice[i]);
				const freq={ one:0,two:0,three:0,attack:0,energy:0,heart:0 };
				keptFaces.forEach(f=>{ if(freq.hasOwnProperty(f)) freq[f]++; });
				let formedValue=0;
				['one','two','three'].forEach(n=>{
					if (freq[n]>=3) formedValue += CFG.formedSetBase + (n==='three'?30: n==='two'?15:10);
					else if (freq[n]===2) formedValue += CFG.pairScoreBase + (n==='three'?18: n==='two'?10:5);
				});
				let specialUtility=0;
				specialUtility += freq.attack * 20;
				specialUtility += freq.energy * (state.phase==='early'?18: state.phase==='mid'?12:8);
				if (!state.player.isInTokyo) specialUtility += freq.heart * (state.player.health <= CFG.healingCriticalHP ? 22: 6);
				const improvementEV = (state.rollsRemaining>0)? (state.rollsRemaining * (state.dice.length - br.kept.length) * 0.9):0;
				const score = formedValue + specialUtility + improvementEV;
				return { tag:br.tag, kept:br.kept.slice().sort((a,b)=>a-b), formedValue, specialUtility, improvementEV, score };
			}).sort((a,b)=> b.score - a.score);
			return { evaluated, best:evaluated[0] };
		} catch(e){ return null; }
	}
	...
	_assembleDecision(state, goal, scored, ev, rollsRemaining, player){
		...
		if (!primaryFace){
			const sorted=state.numbers.slice().sort((a,b)=> b.count - a.count || b.face.localeCompare(a.face));
			primaryFace = sorted.length? sorted[0].face:null;
		}
		const formedFaces=new Set(state.numbers.filter(n=>n.formed).map(n=>n.face));
		const hasFormed=formedFaces.size>0;

		// Keep all formed sets & primary pairs
		state.numbers.forEach(n=>{
			if (n.formed){
		...
		// Free up a die if gridlocked early
		if (rollsRemaining>=2){
			const unformedPairs=state.numbers.filter(n=> n.pair && !n.formed);
			if (unformedPairs.length>0 && keep.size===state.dice.length){
				const specials=[]; state.dice.forEach((f,i)=>{ if(!['one','two','three'].includes(f)) specials.push({face:f,index:i}); });
				if (specials.length){
					const pr={ heart:1, energy:2, attack:3 };
					specials.sort((a,b)=> (pr[a.face]||5)-(pr[b.face]||5));
					keep.delete(specials[0].index); releasedIndices.push(specials[0].index);
				}
			}
		}

		// Consider special faces based on scores
		...
		if (goal && goal.type==='numberSet' && ['one','two','three'].includes(goal.face)){
			...
			if (goalCount < 3 && rollsRemaining>0){
				...
				if (freeDice<2){
					...
					for (const sp of keptSpecials){ if (freeDice>=2) break; keep.delete(sp.index); releasedIndices.push(sp.index); freeDice++; }
					if (freeDice<2){
						const secondaryPairs=['one','two','three'].filter(f=> f!==goal.face && counts[f]===2).sort((a,b)=>({one:1,two:2,three:3})[a]-({one:1,two:2,three:3})[b]);
						for(const sf of secondaryPairs){ if (freeDice>=2) break; const idxs=[]; state.dice.forEach((f,i)=>{ if(f===sf) idxs.push(i); }); if (idxs.length===2){ const rel=idxs[1]; if (keep.delete(rel)){ releasedIndices.push(rel); freeDice++; } } }
					}
					if (goalCount<3){
						let freeNow=state.dice.length - Array.from(keep).length;
						if (freeNow===0){
							const candidates=[];
							state.dice.forEach((f,i)=>{ if (keep.has(i) && f!==goal.face){ let priority=10; if(['one','two','three'].includes(f)){ priority={one:1,two:2,three:3}[f]||9; } else { priority={ heart:0, energy:1, attack:2 }[f] ?? 8; } candidates.push({index:i,face:f,priority}); } });
							candidates.sort((a,b)=> a.priority - b.priority);
							if (candidates.length){ const rel=candidates[0]; if (keep.delete(rel.index)){ releasedIndices.push(rel.index); freeNow++; } }
						}
					}
				}
			}
		...
		let kept=Array.from(keep).sort((a,b)=>a-b);
		...
		if (rollsRemaining>0 && unresolvedPairFaces.length>0 && freeDice===0){
			...
			const releaseOrder=[];
			state.dice.forEach((f,i)=>{ if (secondaryPairFaces.includes(f)) releaseOrder.push({index:i,type:'secondaryPair'}); });
			const spriority={ heart:1, energy:2, attack:3 };
			state.dice.forEach((f,i)=>{ if(!['one','two','three'].includes(f) && keep.has(i)) releaseOrder.push({index:i,type:'special',pr:spriority[f]||5}); });
			releaseOrder.sort((a,b)=>{ if (a.type!==b.type) return a.type==='secondaryPair'?-1:1; return (a.pr||0)-(b.pr||0); });
			const targetFree = rollsRemaining===1 ? 2:1;
			for(const cand of releaseOrder){ if (freeDice>=targetFree) break; if (keep.delete(cand.index)){ releasedIndices.push(cand.index); freeDice++; } }
		}
				...
				pairMeta.forEach(pm=>{
					if (pm.face===primaryFace) return;
						...
						if (keptIdxs.length===2){
							const releaseIndex=keptIdxs[keptIdxs.length-1]; keep.delete(releaseIndex); releasedIndices.push(releaseIndex);
						}
				});
			}
		}
		...
		if (branchSim && branchSim.best){
			const currentSpecialUtility = keptFinal.reduce((acc,idx)=>{
				...
				if (face==='heart' && !state.player.isInTokyo && state.player.health < state.player.maxHealth)
					return acc + (state.player.health<=CFG.healingCriticalHP?9:4);
				return acc;
			},0);
			const currentScore = (ev.total * (CFG.formedSetBase*0.05)) + currentSpecialUtility;
			if (branchSim.best.score > currentScore * 1.08){
				keptFinal = branchSim.best.kept; releasedIndices.push('branch-adjust');
			}
		}
		...
		if (releasedIndices.length){
			heuristicNote = ` | heuristic: freed ${releasedIndices.length} die${releasedIndices.length>1?'s':''}`;
		}

		const improvingFaces=[]; const trialCount=freeDice * state.rollsRemaining;
		if (freeDice>0 && state.rollsRemaining>0){
			state.numbers.forEach(n=>{
				if (n.count<3){
					// Probability of improving this face at least once in remaining rolls
					// approximate using binomial tail
					const needed=1; let p=0;
					for(let x=needed;x<=trialCount;x++){
						p += choose(trialCount,x)*Math.pow(1/6,x)*Math.pow(5/6,trialCount-x);
					}
					improvingFaces.push({face:n.face,p});
				}
			});
			if (goal && !improvingFaces.find(f=>f.face===goal.face)){
				let pGoal=0; for(let x=1;x<=trialCount;x++){ pGoal += choose(trialCount,x)*Math.pow(1/6,x)*Math.pow(5/6,trialCount-x); }
		...
		if (improvingFaces.length){
			// Combine independent improvement events (approx) using log probabilities
			let logNo=0;
		...
		const evClause = action==='endRoll'
			? `marginal gain ${ev.total.toFixed(2)} < threshold ${adaptiveEVThreshold.toFixed(2)}`
			: `projected gain ${ev.total.toFixed(2)} ≥ threshold ${adaptiveEVThreshold.toFixed(2)}`;
		let reasonSentence = explanationFragments.length
			? `${evClause} while ${explanationFragments.join(', ')}`
			: evClause;
		...
		if (player.isInTokyo){
			...
			if (lowHP && threateningEnemies>=2){
				yieldSuggestion=true; reasonParts.push('consider yielding Tokyo');
			}
		}

		const evBreakdown = ev.items.map(it=>({ face:it.face, type:it.type, ev:+it.ev.toFixed(3) }));
		const branchAnalysis = branchSim? {
			best: branchSim.best? {
				tag: branchSim.best.tag,
				score: branchSim.best.score,
				kept: branchSim.best.kept,
				improvementEV: branchSim.best.improvementEV,
				specialUtility: branchSim.best.specialUtility
			}:null,
			considered: branchSim.evaluated.slice(0,8)
		}: null;
		if (branchAnalysis?.best && !reasonParts.join(' ').includes('branch')){
			reasonParts.push(`branch ${branchAnalysis.best.tag} score ${branchAnalysis.best.score}`);
		}
		const projection=this._projectTwoRollEV(state, new Set(keptFinal));
		const t1=performance.now();
		AIDecisionProfiler.record('assembleDecision', t1-t0);

		return {
			action,
			keepDice: keptFinal,
			reason: reasonParts.join(' ') + heuristicNote,
			confidence,
			yieldSuggestion,
			techMeta: {
				...
				adaptiveEVThreshold,
				adaptiveMinKept,
				extraDie: state.cardSummary.extraDie||0
			},
			goal,
			releasedIndices,
			improvementChance:+improvementChance.toFixed(3),
			improvingFaces:Array.from(improvingFaces),
			evBreakdown,
			improvementEV:ev.total,
			branchAnalysis,
			projection
		};
	}
	...
	_enforceInvariants(decision, state, rollsRemaining){
		...
		['one','two','three'].forEach(face=>{
			if (counts[face]===2){
				...
				if (kept.length===1){ idxs.forEach(i=> decision.keepDice.push(i)); decision.reason+=' | inv:pair restore'; if (typeof window!=='undefined') window.NewAIOverrideStats.pairProtect++; }
			}
		});
		// Multi-pair early-stop guard
		...
		// Force at least one free die if gridlocked and still rolling
		if (decision.action==='reroll' && rollsRemaining>0){
			...
			if (freeDiceNow===0 && pairFaces.length>0){
				let primaryFace=null; ['three','two','one'].forEach(f=>{ if (counts[f]===2 && primaryFace===null) primaryFace=f; });
				const releaseCandidates=[];
				['one','two','three'].forEach(f=>{ if (f!==primaryFace && counts[f]===2){ state.dice.forEach((face,i)=>{ if(face===f) releaseCandidates.push({i,pr:1}); }); } });
				...
				if (releaseCandidates.length){ const rel=releaseCandidates[0]; keptSet.delete(rel.i); decision.keepDice=Array.from(keptSet).sort((a,b)=>a-b); decision.reason+=' | inv:forced free die'; }
			}
		}
		// Attack cluster reconsideration
	...
	_extractCardFeatures(card){
		const features=new Set(); if(!card) return features;
	...
	_computeSynergyMultiplier(existingFeatures, newFeatures){
		...
		newFeatures.forEach(nf=>{
			existingFeatures.forEach(ef=>{
				const row=matrix[ef]; if (row && row[nf]) mult*=row[nf];
				const row2=matrix[nf]; if (row2 && row2[ef]) mult*=row2[ef];
			});
		});
		return mult;
	}
	...
	_getDiminishingFactor(feature, count){
		if (count<=0) return 1.0; if (count===1) return 0.82; if (count===2) return 0.6; if (count===3) return 0.45; return 0.35;
	}
	optimizePowerCardPortfolio(availableCards, player){
		...
		const computeBase=(features)=>{ let v=0; features.forEach(f=> v+=(baseWeights[f]||5)); return v; };
		const cardMeta=availableCards.map(c=>{ const feats=this._extractCardFeatures(c); return { card:c, features:feats, cost:c.cost||0 }; });
		const rationalePerCard=[]; const chosenCards=[]; let spent=0; let cumulativeScore=0;
		const simulatedCounts={ ...ownedCounts }; const simulatedFeatures=new Set(ownedFeaturesSet);
		while(true){
			let best=null;
			for(const meta of cardMeta){
				...
				if (meta.features.has('heal') && player.health <=4) personalityMult *= 1.25;
				...
				if (!best || net > best.net) best={ meta, base, diminishing, synergyMult, personalityMult, rawAdjusted, net };
			}
			if (!best || best.net<=5) break; // diminishing return threshold
			chosenCards.push(best.meta.card); spent+=best.meta.cost; cumulativeScore+=best.rawAdjusted;
			best.meta.features.forEach(f=>{ simulatedCounts[f]=(simulatedCounts[f]||0)+1; simulatedFeatures.add(f); });
		...
		if (Object.keys(featureTally).length){
			strategyLabel = Object.entries(featureTally).sort((a,b)=> b[1]-a[1])[0][0];
		}
	...
	evaluateDefensiveCardPurchase(card, player){
		if(!card) return { shouldBuyDefensively:false, defensiveValue:0, denialReason:'' };
		const name=(card.name||'').toLowerCase();
		let defensiveValue=0; let reasonParts=[];
		if (name.includes('head')){ defensiveValue+=8; reasonParts.push('deny extra die'); }
		if (name.includes('brain')){ defensiveValue+=7; reasonParts.push('deny reroll'); }
		...
		const highVP=threats.some(t=> t.victoryPoints>=15); if (highVP) defensiveValue*=1.2;
		const lowHPEnemy=threats.some(t=> t.health<=3); if (lowHPEnemy && (name.includes('heal')||name.includes('regeneration'))) defensiveValue*=0.6;
	...
	explain(decision){
		...
		if (decision.goal) parts.push(`Goal: ${decision.goal.face}`);
		if (decision.improvementEV!=null) parts.push(`EV ${decision.improvementEV.toFixed(2)}`);
		if (decision.improvementChance!=null) parts.push(`Improve ${(decision.improvementChance*100).toFixed(1)}%`);
		if (decision.branchAnalysis?.best){ parts.push(`Branch ${decision.branchAnalysis.best.tag}`); }
		parts.push(`Conf ${(decision.confidence*100).toFixed(0)}%`);
		return parts.join(' | ');
	}
}
